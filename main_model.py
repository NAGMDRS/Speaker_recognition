...
class ECAPAModel(nn.Module):
    ...
    def __init__(self, C, n_class, m, s, lr=0.001, lr_decay=0.97, test_step=1, **kwargs):
        ...
        self.speaker_encoder = ECAPA_TDNN(C=C).to(cfg.DEVICE)  
        self.speaker_loss = AAMsoftmax(n_class=n_class, m=m, s=s).to(cfg.DEVICE)  
    ...
    def train_network(self, epoch, loader):
        ...
        for num, (data, labels) in enumerate(loader, start=1):  
            self.zero_grad()  
            data, labels = data.to(cfg.DEVICE), labels.to(torch.long).to(cfg.DEVICE)  
    ...
    def eval_network(self, eval_list=cfg.EVAL_LIST, eval_path=cfg.DATA_PATH):  
        """  
        ...
        for idx, file in tqdm.tqdm(enumerate(setfiles), total=len(setfiles)):  
            ...
            data_1 = torch.FloatTensor(numpy.stack([audio], axis=0)).to(cfg.DEVICE)  
            ...
            feats = numpy.stack(  
                ...
                axis=0)  
            data_2 = torch.FloatTensor(feats).to(cfg.DEVICE)  
